...
import * as fromUserSelectors from '../../store/user.selectors';
...
export class HomeComponent implements OnInit {
  ...
  constructor(
    ...
    private store: Store<fromUserReducer.UsersState>
  ) {
    this.usersResponse$ = this.store.pipe(select(fromUserSelectors.selectAllUsers));
    this.isLoading$ = this.store.pipe(select(fromUserSelectors.selectUsersIsLoading));
    this.isAdding$ = this.store.pipe(select(fromUserSelectors.selectUserIsAdding));
  }
